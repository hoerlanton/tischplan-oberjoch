...
@Component({
  ...
  templateUrl: 'panoramaRestaurant1.component.html',
  styleUrls: ['../../tischplan.component.css']
})
export class PanoramaRestaurant1Component implements OnInit {
  ...
  transform(tables: Array<any>, term: any) {
    ...
    if (term == "") {
      ...
      if (Array.isArray(tables) && tables.length && term && term.length) {
        this.tablesPanoramaRestaurant1 = tables.filter(item => {
          console.log(item);
