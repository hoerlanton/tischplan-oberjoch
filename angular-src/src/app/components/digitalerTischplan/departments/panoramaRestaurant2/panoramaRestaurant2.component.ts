...
@Component({
  ...
  templateUrl: 'panoramaRestaurant2.component.html',
  styleUrls: ['../../tischplan.component.css']
})
export class PanoramaRestaurant2Component implements OnInit {
  ...
  transform(tables: Array<any>, term: any) {
    ...
    if (term == "") {
      ...
      if (Array.isArray(tables) && tables.length && term && term.length) {
        this.tablesPanoramaRestaurant2 = tables.filter(item => {
          console.log(item);
